...
public class ServiceTypeSelectTag extends SimpleTagSupport {
    ...
    public void doTag() throws JspException, IOException {
        try {
        	...
        	List<DictServiceTypes> optionList = DictUtil.getServiceTypes();

            StringBuffer serviceTypeSelect = new StringBuffer();
            serviceTypeSelect.append("<select id = \"serviceTypeSelect\" name=\"serviceType\" class=\"form-control\">");
            ...
            for(int i = 0;  i<optionList.size();  i++) {
                ...
                serviceTypeSelect.append("<option value='" + item.getId() + "' " + selected + ">" + item.getName() + "</option>");
            }
