...
public class MsgController extends AdminController {
	...
	public String orderDetail(HttpServletRequest request, @RequestParam(value = "id") Long msgId, Model model) throws ParseException {
	...
	public AppResultData<Object> adForm(Model model, @ModelAttribute("contentModel") OaMsgVo msgVo, BindingResult result, HttpServletRequest request)
			throws IOException {
		 ...
		 * 3. 是否是 定时任务
		 * 
		 * 4. 1> 针对 立即发送的 信息, 做发送操作 2> 定时发送, 交给定时任务
		 */
		...
		if (sendWay == 0) {
			...
			for (GroupUser gu : groupAdmins) {
				// 异步推送 给 测试 人员（运营部），消息
				noticeAsyncService.pushMsgToDevice(gu.getUserId(), title, content, msgVo.getCategory(), msgVo.getAction(), msgVo.getParams(),
						msgVo.getGotoUrl());
			}
		...
		Long selectUserType = msgVo.getUserType();
