...
public class InsuranceBaseController extends BaseController {
	...
	public String userTokenList(HttpServletRequest request, Model model, CompanySettingSearchVo searchVo) {
		...
		for (int i = 0; i < lists.size(); i++) {
			...
			settingVo = (InsuranceBaseVo)InitBeanUtil.initObject(settingVo);
			if (setting.getSettingValue() != null) {
				JSONObject setValue = (JSONObject) setting.getSettingValue();
				settingVo = JSON.toJavaObject(setValue, InsuranceBaseVo.class);
			}
	...
	public String submitInsuranceForm(HttpServletRequest request, @ModelAttribute("contentModel") InsuranceBaseVo settingVo, BindingResult result) {
