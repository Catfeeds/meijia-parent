...
<link rel="stylesheet"
   ...
   <script type="text/javascript" src="<c:url value='/js/jquery.toolbarlite.js?ver=10' />" type="text/javascript"></script>
   <script type="text/javascript" src="<c:url value='/js/app.js'/>" type="text/javascript"></script>
	               ...
	               { link: true, display: "创建同级节点", css: "icon-plus", showIcon: true, url: "javascript:;",
	                   click: function() {
	                       ...
	                       if(parentId != undefined)
	                           location.href = "<%= UrlHelper.resolveWithReturnUrl("/authority/add/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), "expanded={1}", pageContext)%>".replace("{0}", parentId).replace(escape("{1}"), expanded);
	                       else
	                           alert("必须选择择一个节点！");
	                       return false;
	                  }
	               ...
	               { link: true, display: "创建子级节点", css: "icon-plus", showIcon: true, url: "javascript:;",
	                   click: function() {
	                       ...
	                       if(selectedId != undefined)
	                       	location.href = "<%= UrlHelper.resolveWithReturnUrl("/authority/add/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), "expanded={1}", pageContext)%>".replace("{0}", selectedId).replace(escape("{1}"), expanded);
	                       else
	                           alert("必须选择择一个节点！");
	                       return false;
	                   }
