	...
	<div class="row">
		<div class="col-lg-12">
				...
				<tbody>
					<c:forEach items="${contentModel.list}" var="item">
						<tr>
							...
							<td><serviceTypeTag:servicetype
									serviceType="${item.serviceType }" /></td>
							<td><timestampTag:timestamp patten="yyyy-MM-dd"
							...
							<td><c:if test="${ item.orderStatus < 2 }">
									<orderStatusTag:orderstatus orderStatus="${ item.orderStatus }" />
								</c:if> <c:if test="${ item.orderStatus == 2 }">
							...
							<td>${ item.orderPay }</td>
							<td><payTypeNameTag:payType payType="${ item.payType }"
									orderStatus="${ item.orderStatus }" /></td>
							<%-- <td>${item.cardPasswd }</td> --%>
							<%-- 	<td>${item.remarks }</td> --%>
							<%-- <td>
			...
			<c:import url="../shared/paging.jsp">
				...
				<c:param name="urlAddress" value="/order/orderViewVoList" />
			</c:import>
		</div>
	</div>
