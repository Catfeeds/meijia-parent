...
var formVal = $('#appCardType-form').validate({
	...
	invalidHandler: function (event, validator) { //display error alert on form submit
		$('.alert-error', $('#appCardType-form')).show();
	},
...
$('.appCardType-form input').keypress(function (e) {
	if (e.which == 13) {
		$("#adForm_btn").click();
		return false;
	}
...
$("#adForm_btn").click(function() {
	if (confirm("确认要保存吗?")) {
		if ($('#appCardType-form').validate().form()) {
			
			$('#appCardType-form').submit();
		}
	}
});
