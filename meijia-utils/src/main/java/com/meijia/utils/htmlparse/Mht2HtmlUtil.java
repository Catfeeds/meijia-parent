...
//		Mht2HtmlUtil.mht2html(mhtPath, htmlPath);
	...
	/**
	 * 将mht文件转为html内容
	 * @param mhtPath mht文件路径
	 * @return String 转为html后的内容
	 */
	public static String mht2html(String mhtPath) {
		try {
			InputStream fis = new FileInputStream(mhtPath);
...
//			e.printStackTrace();
	 ...
	 * @param encoding
	 */
	public static boolean mht2html(String s_SrcMht, String s_DescHtml, String encoding) {
		try {
			...
			if (content instanceof Multipart) {
				...
				// 最后保存HTML文件
				return SaveHtml(strText, s_DescHtml, encoding);
			} else {
				return false;
			}
		} catch (Exception e) {
