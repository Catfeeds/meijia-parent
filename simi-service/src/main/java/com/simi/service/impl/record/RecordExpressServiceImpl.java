...
public class RecordExpressServiceImpl implements RecordExpressService {
	...
	public RecordExpress initRecordExpress() {
		...
		record.setIsCloseTime(0L);
		record.setIsCloseUserId(0L);
	...
	@Override
	public PageInfo selectByListPageVos(RecordExpressSearchVo searchVo, int pageNo, int pageSize) {
		PageHelper.startPage(pageNo, pageSize);
		List<RecordExpressXcloudVo> listVo = new ArrayList<RecordExpressXcloudVo>();
		List<RecordExpress> list = recordExpressMapper.selectByListPage(searchVo);
		RecordExpress item = null;
		for (int i = 0; i < list.size(); i++) {
			item = list.get(i);
			RecordExpressXcloudVo vo = this.getVo(item);
			list.set(i, vo);
		}
		PageInfo result = new PageInfo(list);
		return result;
	...
	public RecordExpressXcloudVo getVo(RecordExpress item) {
		...
		if (item.getExpressType() == 0) vo.setExpressTypeName("收件");
		if (item.getExpressType() == 1) vo.setExpressTypeName("寄件");
		
		vo.setPayTypeName("");
		if (item.getExpressType() == 0) vo.setPayTypeName("公费");
		if (item.getExpressType() == 1) vo.setPayTypeName("自费");
		
		vo.setIsDoneName("");
		if (item.getIsDone() == 0) vo.setIsDoneName("在路上");
		if (item.getIsDone() == 1) vo.setIsDoneName("已送达");
		
		vo.setIsCloseName("");
		if (item.getIsClose() == 0) vo.setIsCloseName("未结算");
		if (item.getIsClose() == 1) vo.setIsCloseName("已结算");
