...
var tagsListSuccess = function(data, textStatus, jqXHR) {
	...
	if (aaa != null) {
		for (var i = 0; i < aaa.length; i++) {
			...
			if (hasSelected) {
				if (i % 2 == 0) {
					tagHtml += "<a href='#' id='tag_id_'"
							+ tags.list[i].tag_id
							+ " onclick='tagClick("
							+ tags.list[i].tag_id
							+ ", $$(this))' class='button button-cancel button-round kehuxq-biaoqian'  style='margin-left: 2%;'>"
							+ tags.list[i].tag_name + "</a>";
				} else {
					tagHtml += "<a href='#' id='tag_id_'"
							+ tags.list[i].tag_id
							+ " onclick='tagClick("
							+ tags.list[i].tag_id
							+ ", $$(this))' class='button button-cancel button-round kehuxq-biaoqian'>"
							+ tags.list[i].tag_name + "</a>";
				}
			} else {
				...
				if (i % 2 == 0) {
					tagHtml += "<a href='#' id='tag_id_'"
							...
							+ " onclick='tagClick("
							+ tags.list[i].tag_id
							+ ", $$(this))' class='button button-cancel button-round kehuxg-biaoqian'  style='margin-left: 2%;'>"
							+ tags.list[i].tag_name + "</a>";
				} else {
					tagHtml += "<a href='#' id='tag_id_'"
							+ tags.list[i].tag_id
							+ " onclick='tagClick("
							+ tags.list[i].tag_id
							+ ", $$(this))' class='button button-cancel button-round kehuxg-biaoqian'>"
							+ tags.list[i].tag_name + "</a>";
				}
...
function orderFormValidation() {
	...
	/*
	 * if (formData.realName == "") { myApp.alert("年龄"); return false; }
	 */
	if (formData.major == "") {
		myApp.alert("请输入所学专业");
		return false;
