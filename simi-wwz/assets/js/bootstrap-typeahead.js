    ...
    Typeahead.prototype = {
        ...
        listen: function () {
            ...
            /*********************
              mzc add
            **********************/
            //if (this.eventSupported('input') ){ ///为毛不对。
                this.$element.on('input propertychange', $.proxy(this.input, this));
            //}
            /*********************
             mzc add over
            **********************/
        ...
        **********************/
        ...
        blur: function (e) {
            ...
            /*********************
                mzc add
                解决iOS （safari） 输入法中文时，选择中文字，其实已经不是键盘操作了，没有键盘事件了，
                所以无法正确响应事件代码，从而无法在keypress 和 keyup事件发生时正确执行this.lookup();
                当选完中文字后，需要按键盘上的'完成'键，以触发输入框内的取消焦点事件，然后在这里调用this.lookup().
            ...
            **********************/
            ...
            setTimeout(function () {
                if (!that.$menu.is(':focus')) {
                  //that.hide();  //mzc 解决iOS safari浏览时，点击软键盘‘完成’按钮，选择列表消失的问题
                }
            }, 150)
        },
        ...
        //
        click: function (e) {
            var active = this.$menu.find('li'); //mzc add
            ...
            /*********************
                mzc add
                解决iOS safari 点击列表后无法真正选中所选项的问题，即解决无法将所选li的class设为active的问题。
            **********************/
            if(e.target.parentNode.className!=='active'){
                //alert('for ios tap select one item');
                for (var ii=0; ii<active.length; ++ii){
                    if(active[ii].className=='active'){
                        //alert(ii);
                        active[ii].className='';
                        break;
                    }
