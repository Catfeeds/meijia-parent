...
public class OrderPayController extends BaseController {
	...
	public AppResultData<Object> postPay(@RequestParam("mobile")
			String mobile, @RequestParam("order_id")
			int order_id, @RequestParam("order_no")
			String order_no, @RequestParam("pay_type")
			int pay_type, @RequestParam("card_passwd")
			String card_passwd, @RequestParam("score")
			int score) {

		
		Orders orders = orderQueryService.selectByPrimaryKey(Long
				.valueOf(order_id));
		OrderPrices orderPrices = orderPricesService.selectByOrderId(Long
				.valueOf(order_id));
...
//	            如果不匹配，返回提示信息：“兑换码不正确哦！”
		...
		AppResultData<Object> result = new AppResultData<Object>(Constants.SUCCESS_0, ConstantMsg.SUCCESS_0_MSG, ordersDetailVo);
		return result;
	}
	public static void main(String[] args) {
		if(new BigDecimal(100).compareTo(new BigDecimal(1))>0) {
