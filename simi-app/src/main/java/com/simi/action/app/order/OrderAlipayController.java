...
public class OrderAlipayController extends BaseController {
	...
	// 7. 订单在线支付成功同步接口
	/**
	 * 订单在线支付成功同步接口 mobile true string 手机号 order_no true string 订单号 pay_type
	 * true int 0 = 余额支付 1 = 支付宝 2 = 微信支付 3 = 智慧支付 4 = 上门刷卡（保留，站位） notify_id
	 * true string 通知ID notify_time true string 通知时间 trade_no true string 流水号
	 * trade_status true string 支付状态 支付宝客户端成功状态为: TRADE_FINISHED 或者为
	 * TRADE_SUCCESS 支付宝网页版成功状态为: success 微信支付成功的状态为: SUCCESS
	 */
	...
	public AppResultData<Object> cardOnlinePay(
		...
		if (orders != null
				&& orders.getOrderStatus() == Constants.ORDER_STATS_2_PAID) {
			...
			if (orderPayService.updateOrderByAlipay(orders, orderPrices,
					updateTime, Constants.ORDER_STATS_2_PAID, payType, tradeNo, payAccount) > 0) {
				return result_success;
			} else {
				return result_fail;
			}
