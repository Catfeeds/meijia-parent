...
public class FeedQueryController extends BaseController {
	...
	/**
	 *  @param user_id  			用户ID
	 *  @param feed_from 			0 = 用户及好友所有的动态  1= 用户发布的动态
	 *
	...
	public AppResultData<Object> getFeedList(
		...
		if (feedFrom.equals((short)0)) {
			//获得所有的好友列表
			 List<UserFriends> userFriends = userFriendService.selectByUserId(userId);
		...
		if (!feeds.isEmpty()) {
			feedList = feedService.changeToFeedListVo(feeds);
